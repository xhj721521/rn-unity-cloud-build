## 近期改动记录
- 2025-11-12\t团队页布局精调：指挥台改为整块玻璃卡、功能卡文案/内框更新，成员列表改为可滚动双列卡片+占位框，并将团队语言改成团队聊天入口。
# 近期改动记录
- 2025-11-12	修整团队页：吸顶卡加入队伍人数+语言气泡，四个功能卡重新排版，仓库面板复用5列网格，成员列表改为两列卡片+空位，团队语言弹出输入框，并修复仓库/公告段落空白。
- 2025-11-11	仓库页改成固定 5 列方格背包：去掉多选模式，容器实时读取宽度传给 InventoryGrid，确保双层描边格子紧贴外框且底部无黑边；空槽位保留  占位。
 开发计划
## 阶段划分与目标
| 阶段 | 目标 | 关键产出 | 预计时长 |
| --- | --- | --- | --- |
...
## 近期改动记录
- 2025-11-12 Invite 模块骨架完成：新增 InviteHome/PosterWorkshop 两页，加入 KPI 卡片、奖励进度、固定高成员列表与海报工坊画布占位；mock 数据与海报模板资产就绪，交互仅本地占位。
- 2025-11-12 团队页 UI 收敛：指挥台卡下移留白、行动入口改为 2×2 胶囊按钮、成员列表改为 FlashList 滚动，底部双按钮固定且留出 tabbar 缓冲。
- 2025-11-12 我的邀请页面细化：KPI 收纳为一张玻璃卡、奖励卡内置“查看奖励 / 去设计海报”按钮并支持展开奖励列表，成员列表改成可滚动 FlatList，过滤/搜索/排序文本统一中文。
- 2025-11-12 首页底栏/字体调性统一：底部导航改为玻璃条 + 胶囊按钮，图标与文字使用统一渐变/主色，同时清理所有 SVG 标注与文案，提升整体大厂质感。
- 2025-11-12 首页背景视频压缩：用 ffmpeg 生成 `shouyeshiping_loop.mp4`（720×1280@20fps，CRF24，0.5s crossfade 无缝循环），体积由 13MB 降至约 4.7MB，适合全页背景与低功耗播放。
...
- 2025-11-10：团队页改为 Roster-First 布局，新增 team tokens、GlassCard/CTA/Badge/Stat/Progress 共享组件及 TeamActionStrip、RosterGrid、RaidWideCard、ChatDock 等模块，接入 mock JSON 与 Drawer、Dock 动效。
